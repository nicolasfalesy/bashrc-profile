...
# BLE.SH EARLY INIT
###############################################################################
[[ $- == *i* ]] && [[ -z ${BLE_VERSION-} ]] && source ~/.local/share/blesh/ble.sh --noattach
iatest=$(expr index "$-" i)
...
###############################################################################
# ALIASES — PACKAGE MANAGEMENT (nala)
###############################################################################
alias sudo='sudo '
alias apt='sudo nala'
...
alias nfi='sudo nala install --update '
alias nf='sudo nala full-upgrade -y'
alias nu='nf'
...
alias notes='vim /home/nick/Nextcloud/constant\ notes.txt'
...
# Tab completion for size
_size_completions() {
    local cur="${COMP_WORDS[COMP_CWORD]}"
    local prev="${COMP_WORDS[COMP_CWORD-1]}"

    if [[ "$prev" == "--size" ]]; then
        COMPREPLY=($(compgen -W "K M G T" -- "$cur"))
        return
    fi

    if [[ "$cur" == -* ]]; then
        COMPREPLY=($(compgen -W "-h --help -H --size" -- "$cur"))
        return
    fi
...
# Tab completion for rcon/rc — Minecraft commands
_rcon_completions() {
    local cur="${COMP_WORDS[COMP_CWORD]}"
    local commands="advancement ban ban-ip banlist clear clone damage data datapack
        ...
        list locate loot me msg op pardon pardon-ip particle perf place playsound
        publish random recipe reload return ride run say schedule scoreboard seed
        setblock setidletimeout setworldspawn spawnpoint spectate spreadplayers stop
        stopsound summon tag team teleport tell tellraw tick time title tm tp trigger
...
###############################################################################
# FUNCTIONS — PREREQUISITES INSTALLER
###############################################################################
# NOTE TO SHELL-FUNCTION-CREATOR AGENT: When adding new functions that require
# external tools, update the install_prereqs() function below to include them.
...
install_prereqs() {
    ...
    case "$1" in
        -h|--help|-H)
            cat <<'HELP'
Install all prerequisites for the ~/.bashrc profile to work fully.

Usage: install_prereqs [-h|--help]
       prereqs
...
Examples:
  install_prereqs        # install everything that's missing
...
HELP
    ...
    # ── Colors ────────────────────────────────────────────────────────────
    local G='\033[0;32m'   # Green  (installed / success)
    local Y='\033[1;33m'   # Yellow (installing)
    local R='\033[0;31m'   # Red    (failure)
    local B='\033[0;34m'   # Blue   (header)
    local NC='\033[0m'     # Reset

    echo -e "${B}--- Installing bash profile prerequisites ---${NC}"
    echo ""
    ...
    # ── Helper: check + install an apt package ────────────────────────────
    _prereq_apt() {
        local pkg="$1"
        local cmd="${2:-$1}"   # command to check (defaults to package name)

        if command -v "$cmd" &>/dev/null; then
            echo -e "  ${G}[skip]${NC} $pkg  (already installed)"
            (( skipped++ ))
            skipped_list+="  $pkg\n"
        else
            echo -e "  ${Y}[install]${NC} $pkg ..."
            if sudo nala install -y "$pkg" 2>/dev/null || sudo apt-get install -y "$pkg" 2>/dev/null; then
                echo -e "  ${G}[ok]${NC} $pkg installed"
                (( installed++ ))
                installed_list+="  $pkg\n"
            else
                echo -e "  ${R}[fail]${NC} $pkg" >&2
                (( failed++ ))
                failed_list+="  $pkg\n"
            fi
        fi
    ...
    # ── 1. Bootstrap nala (must come first so other installs can use it) ──
    echo -e "${B}[1/3] Bootstrapping nala...${NC}"
    if command -v nala &>/dev/null; then
        echo -e "  ${G}[skip]${NC} nala  (already installed)"
        (( skipped++ ))
        skipped_list+="  nala\n"
    else
        echo -e "  ${Y}[install]${NC} nala ..."
        if sudo apt-get install -y nala; then
            echo -e "  ${G}[ok]${NC} nala installed"
            (( installed++ ))
            installed_list+="  nala\n"
        else
            echo -e "  ${R}[fail]${NC} nala" >&2
            (( failed++ ))
            failed_list+="  nala\n"
        fi
    fi
    ...
    # ── 2. APT packages ──────────────────────────────────────────────────
    echo -e "${B}[2/3] APT packages...${NC}"
    #              package          command-to-check
    _prereq_apt   fastfetch        fastfetch
    _prereq_apt   neovim           nvim
    _prereq_apt   trash-cli        trash
    _prereq_apt   ripgrep          rg
    _prereq_apt   xclip            xclip
    _prereq_apt   alacritty        alacritty
    _prereq_apt   wireguard-tools  wg
    _prereq_apt   curl             curl
    _prereq_apt   net-tools        netstat
    _prereq_apt   tree             tree
    _prereq_apt   bc               bc
    _prereq_apt   git              git
    ...
    # ── 3. Special installs ──────────────────────────────────────────────
    echo -e "${B}[3/3] Special installs...${NC}"

    # starship
    if command -v starship &>/dev/null; then
        echo -e "  ${G}[skip]${NC} starship  (already installed)"
        (( skipped++ ))
        skipped_list+="  starship\n"
    else
        echo -e "  ${Y}[install]${NC} starship ..."
        if curl -sS https://starship.rs/install.sh | sh -s -- -y; then
            echo -e "  ${G}[ok]${NC} starship installed"
            (( installed++ ))
            installed_list+="  starship\n"
        else
            echo -e "  ${R}[fail]${NC} starship" >&2
            (( failed++ ))
            failed_list+="  starship\n"
        fi
    fi
    ...
    # mcrcon
    if command -v mcrcon &>/dev/null; then
        echo -e "  ${G}[skip]${NC} mcrcon  (already installed)"
        (( skipped++ ))
        skipped_list+="  mcrcon\n"
    else
        echo -e "  ${Y}[install]${NC} mcrcon (build from source) ..."
        local tmpdir
        tmpdir=$(mktemp -d)
        if git clone https://github.com/Tiiffi/mcrcon.git "$tmpdir/mcrcon" && \
           make -C "$tmpdir/mcrcon" && \
           sudo make -C "$tmpdir/mcrcon" install; then
            echo -e "  ${G}[ok]${NC} mcrcon installed"
            (( installed++ ))
            installed_list+="  mcrcon\n"
        else
            echo -e "  ${R}[fail]${NC} mcrcon" >&2
            (( failed++ ))
            failed_list+="  mcrcon\n"
        fi
        rm -rf "$tmpdir"
    ...
    if [[ -f "$HOME/.local/share/blesh/ble.sh" ]]; then
        echo -e "  ${G}[skip]${NC} ble.sh  (already installed)"
        (( skipped++ ))
        skipped_list+="  ble.sh\n"
    else
        echo -e "  ${Y}[install]${NC} ble.sh (build from source) ..."
        local tmpdir2
        tmpdir2=$(mktemp -d)
        if git clone --recursive --depth 1 --shallow-submodules https://github.com/akinomyoga/ble.sh.git "$tmpdir2/ble.sh" && \
           make -C "$tmpdir2/ble.sh" install PREFIX="$HOME/.local"; then
            echo -e "  ${G}[ok]${NC} ble.sh installed"
            (( installed++ ))
            installed_list+="  ble.sh\n"
        else
            echo -e "  ${R}[fail]${NC} ble.sh" >&2
            (( failed++ ))
            failed_list+="  ble.sh\n"
        fi
        rm -rf "$tmpdir2"
    ...
    # ── Summary ──────────────────────────────────────────────────────────
    echo -e "${B}--- Summary ---${NC}"
    echo -e "  ${G}Installed:${NC} $installed"
    if [[ -n "$installed_list" ]]; then
        echo -e "$installed_list"
    fi
    echo -e "  ${G}Skipped (already present):${NC} $skipped"
