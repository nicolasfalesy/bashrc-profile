...
|------|-----|-------------|
| `uw_bashrc` | bashrc for the UW student servers | `profiles/uw.sh` + `starship_uw.toml` (`install.sh --profile uw`) |
| `zshrc` | zsh config used on TrueNAS while its login shell was zsh | the NAS now runs bash: `profiles/nas.sh` (`install.sh --profile nas`) |
